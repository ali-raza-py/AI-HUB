...
Actively developed; MIT license; local-first completion/chat extension (verified Sep 2026).

## Overview

**Twinny** is a editor/ide native agent (vs code extension host) in the *VS Code / IDE AI agents* category, made by Community (twinnydotdev).

Local-model (Ollama) chat and FIM completion in VS Code at zero cost.

## Official Links

- [Official website](https://twinny.dev/)
...
MIT license, local-first, OpenAI-compatible endpoints, workspace embeddings.

Known limitations: Not an autonomous agent; completion/chat focus.
...
- [Official install/setup docs](https://twinny.dev/)
...
Not an autonomous agent; completion/chat focus.
...
- Developers who need stability guarantees — Twinny changes quickly; re-verify features and pricing before committing.
...
Light client; heavy inference happens in the cloud or via BYOK. Local inference (Ollama) on 8 GB RAM is limited to small models (~1–4 B) and will be slow on this CPU-only machine — fine for experimentation, not for daily heavy agent work. Stated requirements: Runs well; local models need RAM for larger sizes..
...
- [Source repository](https://github.com/twinnydotdev/twinny)
