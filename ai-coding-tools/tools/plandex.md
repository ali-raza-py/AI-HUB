...
**Plandex** is a terminal agent (cli/repl) in the *Terminal / CLI coding agents* category, made by Plandex (Danenania).

Large multi-file tasks with reviewable, rollback-able diffs.
...
2M-token context, project maps, plan branches, controlled command execution, model packs.

Known limitations: Windows is WSL-only; cloud offering winding down.
...
Free, open source, BYOK; Plandex Cloud winding down — self-host recommended.
...
Windows is WSL-only; cloud offering winding down.
...
- Developers who need stability guarantees — Plandex changes quickly; re-verify features and pricing before committing.
