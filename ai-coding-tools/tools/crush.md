...
Open-source terminal AI agent (successor to OpenCode) from Charm; FSL-1.1-MIT license; multi-model, MCP, LSP, Windows support (verified Sep 2026).

## Overview

**Crush** is a terminal agent (cli/repl) in the *Terminal / CLI coding agents* category, made by Charm.

Terminal-first agents with maximal model/provider flexibility.

## Official Links

- [Official website](https://github.com/charmbracelet/crush)
...
Multi-model, mid-session model switching, MCP (http/stdio/sse), LSP awareness, cross-platform.

Known limitations: Model/provider catalog auto-updates (Catwalk) — watch data/config churn.
...
- [Official install/setup docs](https://github.com/charmbracelet/crush)
...
Model/provider catalog auto-updates (Catwalk) — watch data/config churn.
...
- Developers who need stability guarantees — Crush changes quickly; re-verify features and pricing before committing.
...
Light client; heavy inference happens in the cloud or via BYOK. Local inference (Ollama) on 8 GB RAM is limited to small models (~1–4 B) and will be slow on this CPU-only machine — fine for experimentation, not for daily heavy agent work. Stated requirements: Any; light Go binary..
...
- [Source repository](https://github.com/charmbracelet/crush)
