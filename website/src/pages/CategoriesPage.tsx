  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="reveal mb-12 flex flex-col justify-between gap-5 border-b border-white/10 pb-8 sm:flex-row sm:items-end">
        <div><div className="eyebrow flex items-center gap-2"><Boxes size={13} /> Organize your search</div><h1 className="mt-4 font-display text-4xl font-semibold text-white sm:text-5xl">Explore by<br /><span className="text-[#f5c542]">category.</span></h1><p className="mt-4 max-w-xl text-base leading-7 text-[#a6a99f]">Start with the kind of work you want to do, then narrow down to the tools that fit.</p></div>
        <Link to="/tools" className="inline-flex items-center gap-2 text-sm font-semibold text-[#f5c542] hover:text-white">See all {tools.length} tools <ArrowRight size={15} /></Link>
      </div>
          ...
          return (
            <section key={cat.id} id={cat.id} className="surface-panel scroll-mt-24 rounded-2xl p-5 sm:p-6 reveal" style={{ animationDelay: `${index * 70}ms` }}>
              <div className="flex flex-col justify-between gap-4 border-b border-white/8 pb-5 sm:flex-row sm:items-start">
                <div>
                  <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-none bg-[#f5c542]/10 font-mono text-sm text-[#f5c542]">0{index + 1}</span><h2 className="font-display text-xl font-semibold text-white">{cat.title || cat.name}</h2></div>
              ...
              <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {catTools.slice(0, 6).map(tool => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 font-semibold text-white group-hover:text-[#f5c542]">
                      {tool.name}
                      <ChevronRight size={15} className="text-[#777b72] transition group-hover:translate-x-1 group-hover:text-[#f5c542]" />
                    </div>
              ...
              {catTools.length > 6 && (
                <Link
                  ...
                  className="mt-5 inline-flex items-center gap-1 text-sm font-semibold text-[#f5c542] hover:text-white"
                >
